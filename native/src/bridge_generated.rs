...
fn wire_layout_graph_impl(
    port_: MessagePort,
    number_of_nodes: impl Wire2Api<u32> + UnwindSafe,
    edges: impl Wire2Api<Vec<RustEdge>> + UnwindSafe,
) {
    FLUTTER_RUST_BRIDGE_HANDLER.wrap(
        WrapInfo {
            debug_name: "layout_graph",
            port: Some(port_),
            mode: FfiCallMode::Normal,
        },
        move || {
            let api_number_of_nodes = number_of_nodes.wire2api();
            let api_edges = edges.wire2api();
            move |task_callback| Ok(layout_graph(api_number_of_nodes, api_edges))
        },
    )
}
fn wire_add_impl(
    port_: MessagePort,
    a: impl Wire2Api<f32> + UnwindSafe,
...
impl support::IntoDart for Position {
    fn into_dart(self) -> support::DartAbi {
        vec![self.x.into_dart(), self.y.into_dart(), self.z.into_dart()].into_dart()
    }
}
